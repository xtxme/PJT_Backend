...
CREATE TABLE `employee` (
	...
	`tel` varchar(50),
	`role` enum('owner','warehouse_staff','sales') DEFAULT 'sales',
	`email` varchar(255),
...
CREATE TABLE `products` (
	...
	`company` varchar(255),
	`status` enum('active','low_stock','restock_pending','pricing_pending') NOT NULL DEFAULT 'active',
	`created_at` timestamp NOT NULL DEFAULT (now()),
	`updated_at` timestamp ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `stock_in` (
	...
	`status` enum('pending','some_received','completed','canceled') NOT NULL DEFAULT 'pending',
	`note` text,
